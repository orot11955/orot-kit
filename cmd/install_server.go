		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if opts.dryRun {
				return writer(cmd).Write(output.Result{
					...
					Command: []string{"kit install-server --addr " + addr + " --bin-dir " + config.BinDir + " --assets-dir " + config.AssetsDir + " --stats-file " + config.StatsFile + " --base-url " + config.BaseURL},
			...
			cmd.Printf("Title\n  Install Server\n\nResult\n  Listening on %s\n  Base URL: %s\n  Binaries: %s\n  Assets: %s\n  Stats: %s\n\n", addr, baseURL, config.BinDir, config.AssetsDir, config.StatsFile)
...
func defaultInstallServerStatsFile() string {
	return "~/.kit-server/download-stats.json"
}
