...
func (s *Server) handleBinary(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == http.MethodGet && !isUpdateDownload(r) {
...
func isUpdateDownload(r *http.Request) bool {
	value := strings.ToLower(strings.TrimSpace(r.URL.Query().Get("update")))
	if value == "1" || value == "true" || value == "yes" {
		return true
	}
	return strings.EqualFold(strings.TrimSpace(r.Header.Get("X-Kit-Update")), "1")
}
